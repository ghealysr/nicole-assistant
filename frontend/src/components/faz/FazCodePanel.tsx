...
interface FazCodePanelProps {
  isOpen: boolean;
  onClose: () => void;
  isFullWidth?: boolean; // When true, renders as main content instead of slide-in
}
...
export function FazCodePanel({ isOpen, onClose, isFullWidth = false }: FazCodePanelProps) {
  ...
  const renderErrorBanner = () => pipelineError && (
  ...
  const renderContent = () => (
    <>
      ...
      <div className="flex-1 overflow-hidden">
        {view === 'projects' ? renderProjectsView() : renderWorkspaceView()}
      </div>
    </>
  );
  ...
  // Full width mode - renders as main content
  if (isFullWidth) {
    return (
      <div className="h-full w-full bg-[#08080C] flex flex-col">
        {renderContent()}
      </div>
    );
  }

  // Slide-in panel mode (legacy)
  return (
    <div
