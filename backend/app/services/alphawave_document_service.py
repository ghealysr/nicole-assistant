...
class AlphawaveDocumentService:
    ...
    async def process_document(
        ...
        try:
            # Insert into uploaded_files (Production schema)
            ...
            # Insert into document_repository (Production schema)
...
..."""
    ...
    async def search_documents(
        ...
        try:
            ...
            # Vector search in Tiger (Production schema)
            rows = await db.fetch(
                """
                SELECT 
                    dc.chunk_id,
        ...
        # Fallback to text search if needed (Production schema)
        ...
        try:
            rows = await db.fetch(
                ...
                SELECT 
                    ...
                    uf.file_name,
                    uf.file_size,
                    (SELECT COUNT(*) FROM document_chunks WHERE doc_id = dr.doc_id) as chunks_count
                FROM document_repository dr
                LEFT JOIN uploaded_files uf ON uf.file_id = dr.file_id
                WHERE dr.user_id = $1
